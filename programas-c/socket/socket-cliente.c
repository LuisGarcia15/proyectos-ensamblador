...
int main(){
    ...
    estructura_socket.sin_port = 6000;
   ...
   int resultado_conexion = connect(fd_socket, (struct sockaddr*)&estructura_socket, 
                             sizeof(estructura_socket));
   if(resultado_conexion < 0){
        printf("Error al realizar la conexion %d\n", resultado_conexion);
   }else{
        printf("Socket conectado con exito\n");
   }
