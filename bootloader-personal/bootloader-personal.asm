; DIRECTIVAS -  Es una operaci\C3\B3n orientada al ensamblador que le indica 
;el curso/guia/comportamiento que debe tomar las instrucciones y/o
;ejecuci\C3\B3n del programa. NO SON EJECUTADAS POR EL CPU. NO GENERAN
;INSTRUCCIONES A NIVEL DE MEMORIA PRINCIPAL

;INSTRUCCIONES - Es una operaci\C3\B3n orientada a ser ejecutada por el CPU, 
;expresada en con un nombre simbolico. Representa un instrucci\C3\B3n de una
;arquitectura que puede ejecutar el CPU.

;LABEL - Es una nombre simbolico para un bloque de c\C3\B3digo, que describe un 
;lugar en programa m\C3\A1s facil que la direcci\C3\B3n de memoria. _, ., ? son suados
;como simbolos de inicio de la etiqueta.

ORG 0 ;Permite especificar la direcci\C3\B3n origen donde NASM asume que el programa inicio.
;desde ah\C3\AD pudes dirigir el movimiento del programa. Hace independiente el c\C3\B3digo de donde
;fue cargado en memoria. Puede ayudar a identificar la zona donde esta la instruccion del inicio
;del archivo
BITS 16 ;Especifica si NASM debe generar c\C3\B3digo generado para correr en una arquitectura
;de procesador 16,32 y 64 bits.
; --------- Formato inicial exigido por el BIOS (https://wiki.osdev.org/index.php?title=FAT#Boot_Record) ---------
;Punto de entrada del programa
;Label representa la direcci\C3\B3n de memoria de una instrucci\C3\B3n o datos
_start:
;jmp transfiere el control del programa a una ubicaci\C3\B3n especifica en el codigo
;pudiendo ser una direcci\C3\B3n de memoria, una label, un registro. Modificando
;el program counter
jmp short start; 2 bytes - El Opcode de JMP mide 8 bits, su operando mide 8 -EXIGIDO POR LA BIOS
nop; 1 byte -EXIGIDO POR LA BIOS
;NoOperation: Intrucci\C3\B3n que le indica que el CPU no haga ninguna
;operaci\C3\B3n por un ciclo. Usado para llenar con espacio la secuencia
;de instrucciones

times 33 nop ; Espacio para el BCP (BIOS CONTROL BLOCK)
;prefijo times que ensambla n veces una instrucci\C3\B3n, dejando ese espacio
;vacio. En este caso con la instrucci\C3\B3n NoOperation

;Label representa la direcci\C3\B3n de memoria de una instrucci\C3\B3n o datos
start:
jmp 0x07C0:main; Ajusta el CS (CODE SEGMENT) a 0x07C0
;Salta al label main. coloca el CS en 0x07C0. IP a main

;Label representa la direcci\C3\B3n de memoria de una instrucci\C3\B3n o datos
main:
cli ; Clear Interrupts - Limpia la bandera de interrupciones - Ignorando
;las interrupciones maskable. Coloca en 0

xor ax, ax ; Get 0 - Evalua el valor de AX consigo mismo en una operaci\C3\B3n XOR
;Obteniendo un valor cero en el registro AX

mov ds, ax ; Reset DATA SEGMENT to 0 - Muevele el valor de AX (Cero) a DATA SEGMENT

; mov es, ax ; Reset EXTRA SEGMENT (heap) to 0

mov ss, ax ; Reset STACK SEGMENT to 0

mov sp, 0x7C00 ; Set STACK POINTER to 0x7C00

;Ajustar inicio del DATA SEGMENT a CODE SEGMENT
push cs ;Envia el valor de CODE SEGMENT al STACK
pop ds ;Saca el valor del stack y lo COLOCA A DATA SEGMENT

push cs ;Envia el valor de CODE SEGMENT al STACK
pop es ;Saca el valor del stack y lo COLOCA A EXTERNAL SEGMENT

cld ;Clear Direction Flag - Determina como un conjunto de bits seran movidos
;de un lugar en la memoria a otro. Causando que SOURCE INDEX Y DESTINAITION
;INDEX aumenten incrementalmente

;Se usa ES:BX. ES es la direccion del segmento. BX es el offset para escribir
;en la direccion de memoria correcta. Almacena los datos en la direccion
;0x7E00 (dirección siguiente al bootloader) y sus subsiguientes celdas
mov bx, 0x0200

mov ah, 0x02 ;Funcion 2 de la interrupcion 13H de la Bios
mov al, 1    ;Numero de segmentos a leer
mov ch, 0    ;Numero de cilindro
mov dh, 0    ;Numero de head
mov cl, 2    ;Numero se sector

int 0x13     ;Llamada a la interrupcion 13 H func 2
    
mov si, 0x0200 ;Asigna el dato en la direccion a Source Index para
;Recorrer los caracteres almacenados del sector 2 en las siguientes
;direcciones de memoria

.print:
;Load Single Byte: Toma el contenido de la dirección de memoria apuntada por el
;registro Data Segment y Source Index. Y lo coloca en AL
    lodsb
    test al, al ; Operacion AND entre al y al guardando las flags de la ALU
    ;Jump If Zero: Evalua la bandera Zero Flag, si contine 1, salta a .done
    jz .done ; Si Zero Flag contiene 1, salta a done

mov ah, 0x0E ; Función 0x0E de la INT 10h: "Teletype Output"
int 0x10 ; Llamada a la interrupción de video del BIOS - Almacena en AL el Caracter
;En ascii para imprimirlo
jmp .print ; Repite para el siguiente carácter - "Brinca a label .print"

.done:
    hlt ; Detiene la ejecucion y pone el CPU en estado inactivo de bajo consumo

;Estructura de MBR - 512 B
; Se ha llenado tantos B, necesito solo 510 B
;$ - Direcci\C3\B3n actual de la instrucci\C3\B3n
;$$ - Devuelve la direcci\C3\B3n de inicio
;A la pocisi\C3\B3n actual restarle la pocici\C3\B3n inicial. Luego restarle a 510 ese valor
;Resultando en un valor M, El resultado rellenarlo de 0 (Ceros)
;Ensamblando M veces con cero. Es a nivel de c\C3\B3digo
times 510-($-$$) db 0
;Define Word (dw): Define un espacio en memoria para almacenas datos | 2 Byte - 16 bits
;Interpreta todo lo que sigue como secuencia de Word. 0 -> EOL: End Of Line, define
;el fin de la cadena para mostrarla en pantalla
dw 0xAA55