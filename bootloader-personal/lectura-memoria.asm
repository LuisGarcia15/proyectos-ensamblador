...
;DS se usa para mover datos en memoria. DS = 0x7C00
mov si, msg_p ; offset de msg en SOURCE INDEX (registro para operaciones en cadena)
mov di, 0x0200

guardar_en_mem:
    lodsb;Load Single Byte: Toma el contenido de la dirección de memoria apuntada por el
    ;registro Data Segment y Source Index. Y lo coloca en AL
    mov [di], al ;Operador de desreferenciación
    ...
    jz .continuar_lectura; Si Zero Flag contiene 1, salta a done
    jmp guardar_en_mem; Salta a continuar

.continuar_lectura:
    ;DS se usa para mover datos en memoria. DS = 0x7C00
    mov si, 0x0200; offset de msg en SOURCE INDEX (registro para operaciones en cadena)

.imprimir_cadena_memoria:
    ;Load Single Byte: Toma el contenido de la dirección de memoria apuntada por el
    ;registro Data Segment y Source Index. Y lo coloca en AL
    lodsb
    test al, al ; Operacion AND entre al y al guardando las flags de la ALU
    ;Jump If Zero: Evalua la bandera Zero Flag, si contine 1, salta a .done
    jz .imprimir_mensaje_final ; Si Zero Flag contiene 1, salta a done

    mov ah, 0x0E ; Función 0x0E de la INT 10h: "Teletype Output"
    int 0x10 ; Llamada a la interrupción de video del BIOS
    jmp .imprimir_cadena_memoria ; Repite para el siguiente carácter - "Brinca a label .print"
